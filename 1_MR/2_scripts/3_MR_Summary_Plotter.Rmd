...
saveRDS(label_tb, '1_MR/3_output/label_tb.rds')
```
```{r}
...
```{r}
summary_plotter <- function(input_tb, label_tb, method_choice = 'Inverse variance weighted|Wald ratio', output_folder = '1_MR/3_output/3_summary_plots/', conf_int=T, point_size= 4){
...
forest_plot_overall <- ggplot(filtered_sig_mr_results)+
  geom_point(aes(x=beta, y= exposure_outcome_label,fill=nonHCM_phenogroup),size = 3, shape = 23)+
  geom_errorbarh(aes(xmin=lo_ci, xmax=up_ci, y=exposure_outcome_label, col=nonHCM_phenogroup),height = 0.1, linewidth = 0.8)+
  geom_vline(xintercept=0, col='black', linetype='dashed')+
  xlab('Change in Outcome per Change in Exposure')+
  ylab('Exposure -> Outcome')+
  labs(fill='Non-HCM Phenotype Group')+
...
forest_plot_hcmexposure <- ggplot(filter(filtered_sig_mr_results, outcome != 'HF', exposure=='HCM') %>%
    mutate(scaled_b = beta*log(2)) %>% #i.e if HCM = exposure, scale so that the beta (originally SD per log-odds(HCM)) is per doubling of Odds(HCM)
    mutate(scaled_lo_ci = lo_ci*log(2),
        scaled_up_ci = up_ci*log(2)
        ))+#Plot this plot without HF signal
  geom_point(aes(x=scaled_b, y= exposure_outcome_label,fill=nonHCM_phenogroup),size = 3, shape = 23)+
  geom_errorbarh(aes(xmin=scaled_lo_ci, xmax=scaled_up_ci, y=exposure_outcome_label, col=nonHCM_phenogroup),height = 0.1, linewidth = 0.8)+
  geom_vline(xintercept=0, col='black', linetype='dashed')+
  xlab('Change in Outcome (SD)\nper Doubling of Odds(HCM)')+
  ylab('Exposure -> Outcome')+
...
forest_plot_hcmoutcome <- ggplot(filter(filtered_sig_mr_results, exposure != 'HF', outcome=='HCM'))+ #Plot this plot without HF signal
  geom_point(aes(x=or, y= exposure_outcome_label, fill=nonHCM_phenogroup),size = 3, shape = 23)+
  geom_errorbarh(aes(xmin=or_lci95, xmax=or_uci95, y=exposure_outcome_label, col=nonHCM_phenogroup),height = 0.1, linewidth = 0.8)+
...
ggsave('1_MR/3_output/3_summary_plots/sig_MR_forestplot_combined_noHF.png',combined_forest_plot, dpi=600, width=6, height=4.5)
...
hfsubgroup_merged_results <- bind_rows(hfsubgroup_merged_results, filter(filtered_sig_mr_results,outcome =='HF'|exposure == 'HF'))%>%
  ...
  mutate(scaled_or = exp(beta*log(2))) %>% #i.e if HCM -> HF, express as OR(HF) per doubling (Odds) of HCM and if HF -> HCM, express as OR(HCM) per doubling (Odds) of HF
  mutate(scaled_or_lowci = exp(lo_ci*log(2)),
        scaled_or_highci = exp(up_ci*log(2))
  )
...
forest_plot_hcmexposure_hf <- ggplot(filter(hfsubgroup_merged_results ,str_detect(outcome,'HF')))+
  geom_point(aes(x=scaled_or, y= exposure_outcome_label, fill = outcome),size = 3, shape = 23)+
  geom_errorbar(aes(xmin=scaled_or_lowci, xmax=scaled_or_highci, y=exposure_outcome_label, col = outcome))+
  geom_vline(xintercept=1, col='black', linetype='dashed')+
  xlab('Odds Ratio (HF) per Doubling of Odds(HCM)')+
  ylab('Exposure -> Outcome')+
  scale_colour_manual(values = hf_colour_map)+
  scale_fill_manual(values = hf_colour_map)+
  theme(legend.position='none')

forest_plot_hcmoutcome_hf <- ggplot(filter(hfsubgroup_merged_results, str_detect(exposure,'HF')))+
  geom_point(aes(x=scaled_or, y= exposure_outcome_label, fill=exposure),size = 3, shape = 23)+
  geom_errorbar(aes(xmin=scaled_or_lowci, xmax=scaled_or_highci, y=exposure_outcome_label, col=exposure))+
  geom_vline(xintercept=1, col='black', linetype='dashed')+
...
ggsave('1_MR/3_output/3_summary_plots/sig_MR_forestplot_combined_HFonly.png',combined_forest_plot_hf, dpi=600, width=6, height=4.5)
```

```{r}
# #N.B To get the numerical values out from the mr_results - see filtered_sig_mr_results
