...
I also repeat the above summary plot but with the discrete variables transformed from Log(Odds Ratio) to Odds Ratio.
...
```{r}
# #Convert all discrete variables to per-OR (instead of Log(Odds Ratio)) exposure and outcome
...
sig_mr_results_or <- sig_mr_results_or %>%
  mutate(exposure_plot_group = case_when(exposure%in% persd_exposure_vars ~ 'Per-SD Exposure', 
                                       exposure == 'HCM' ~ 'Per-Log(Odds Ratio) Exposure',
                                       T~'Per-Unit Exposure'))
...
forest_plot_subgroup<- ggplot(subgroup_mr_results_or)+
  geom_point(aes(x=b, y= forcats::fct_reorder(exposure_outcome, outcome), col=nonHCM_phenogroup))+
  geom_errorbar(aes(xmin=lo_ci, xmax=up_ci, y=exposure_outcome,col=nonHCM_phenogroup))+
  geom_vline(xintercept=0, col='black', linetype='dashed')+
  xlab('Change in Outcome ( log(Odds Ratio) for Discrete & SD for Continuous ) per Change in Exposure')+
  ylab('Exposure -> Outcome')+
