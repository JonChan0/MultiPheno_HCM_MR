...
```{r}
#This function imports in SNPs from the .tsv format from GWAS Catalog OR from .xlsx format from CVD HugeAmp and Supplementary Tables of original publications.
#Its ultimate output  = list of SNPs in rsID format
...
These effects are generally adjusted for via inclusion of genetic PCs in the underlying GWAS itself.
...
This section computes what is a suitable threshold for pleiotropy cutoff.
```{r}
#This is the number of MTC-passed sig. traits from PheWAS of 10k randomly sampled SNPs using Open Target Genetics.
nulldist <- read_rds('1_MR/1_data/pleiotropy_threshold/nulldist_10k.rds')
hist(nulldist) #Very skewed distribution

quantile(nulldist,0.95,na.rm=T)
quantile(nulldist,0.99,na.rm=T)
quantile(nulldist, 0.995, na.rm=T)
quantile(nulldist, 0.999, na.rm=T)
```

This leverages Open Target Genetics CredibleGWASSets ~ PheWAS but with in-built fine-mapping as well to evaluate SNP's pleiotropy and plots out the other traits which the SNP has statistical significant association with (defined by pval_cutoff).

```{r}
source('1_MR/2_scripts/2_OTG_CredibleSetQuery.R')

otg_caller_processor <- function(input_snp){

  # Call OTG
  safe_pheWAS <- safely(get_gwas_credible_sets)
  ...
  if (is.null(otg_phewas_data$result)) { # i.e. no information returned
    ...
    processed_phewas_data <- as_tibble(otg_phewas_data$result$variant$gwasCredibleSets$rows) %>%
      mutate(SNP = input_snp) %>%
      select(-finemappingMethod, -confidence) %>%
      unique()
...
#This applies multiple testing correction to filter for only MTC-significant pleiotropic associations. This is archived as per otargen 2.0 and the switch to Open Targets Platform
...
scatter_plotter <- function(processed_data_tb, pheno, output_path, qval_cutoff=0.05, priority_cutoff=47){
  theme_set(theme_classic())
...
pairwise_LD_checker <- function(input_SNPs,pheno, output_path, token, population='GBR'){ 
...
```{r}
instrument_files <- c('1_MR/1_data/gwas_associations/tadros25_hcm_nonMTAG_associations.xlsx',
                      '1_MR/1_data/gwas_associations/roselli25_af_associations.xlsx',
                      '1_MR/1_data/gwas_associations/keaton24_dbp_associations.tsv',
                      '1_MR/1_data/gwas_associations/sidorenko24_bmi_associations.tsv',
                      '1_MR/1_data/gwas_associations/sakaue21_t2d_associations.tsv',
                      '1_MR/1_data/gwas_associations/liu18_SmokingInitiation_associations.tsv',
                      '1_MR/1_data/gwas_associations/nauffal24_t1time_associations.xlsx',
                      '1_MR/1_data/gwas_associations/thanaj22_maxLAVi_associations.tsv',
                      '1_MR/1_data/gwas_associations/thanaj22_long_dsr_associations.tsv',
                      '1_MR/1_data/gwas_associations/thanaj22_radial_dsr_associations.tsv'
                      )
phenotypes <- c('HCM' ,'AF','DBP','BMI','T2D','SmokingInitiation','T1time','maxLAVi','longDSR','radialDSR'
                )

walk2(instrument_files, phenotypes,~main2(.x,.y,'1_MR/3_output/1_MR_QC/pleiotropy_check/'))
...
```{r}
hcmr_instrument_files <- list.files('1_MR/1_data/gwas_associations/', pattern='hcmr[^\\.]+\\.xlsx', full.names = T)
hcmr_phenotypes <- str_match(basename(hcmr_instrument_files), '([^\\.]+)_associations\\.xlsx')[,2]

ukb_instrument_files <- list.files('1_MR/1_data/gwas_associations/', pattern='ukb[^\\.]+\\.xlsx', full.names = T)
ukb_phenotypes <- str_match(basename(ukb_instrument_files), '([^\\.]+)_associations\\.xlsx')[,2]

walk2(hcmr_instrument_files, hcmr_phenotypes,~main2(.x,.y,'1_MR/3_output/1_MR_QC/pleiotropy_check/'))
walk2(ukb_instrument_files, ukb_phenotypes,~main2(.x,.y,'1_MR/3_output/1_MR_QC/pleiotropy_check/'))
```

Run for subgroup HCMs as well.

```{r}
subgroup_hcm_instrument_files <- c('1_MR/1_data/gwas_associations/tadros25_sarcposhcm_nonMTAG_associations.xlsx',
                                   '1_MR/1_data/gwas_associations/tadros25_sarcneghcm_nonMTAG_associations.xlsx')
subgroup_hcm_phenotypes <- c('sarcposHCM','sarcnegHCM')
walk2(subgroup_hcm_instrument_files,subgroup_hcm_phenotypes,~main2(.x,.y,'1_MR/3_output/1_MR_QC/pleiotropy_check/'))
```

Run for additional phenotypes added from Sep 2025.

```{r}
additional_instrument_files <- c(
  '1_MR/1_data/gwas_associations/aragam22_cad_associations.tsv',
  '1_MR/1_data/gwas_associations/loeb24_eGFR_associations.tsv',
  '1_MR/1_data/gwas_associations/henry25_hf_associations.xlsx',
  '1_MR/1_data/gwas_associations/henry25_HFpEF_associations.xlsx',
  '1_MR/1_data/gwas_associations/henry25_HFrEF_associations.xlsx',
  '1_MR/1_data/gwas_associations/mishra22_stroke_associations.xlsx'
)
additional_phenotypes <- c('CAD','eGFR','HF','HFpEF','HFrEF','stroke')
walk2(additional_instrument_files,additional_phenotypes,~main2(.x,.y,'1_MR/3_output/1_MR_QC/pleiotropy_check/'))
```
...
1) Excess pleiotropy with many other unique traits (threshold = 47 corresponding to 99.5% percentile from a 10k random sample from OTG)
2) Pleiotropy with certain red-flag traits which are causally associated with the outcome.

```{r}

snp_pleio_filter <- function(og_path, pheno, output_path,red_flags='',pheno2='', n_cutoff=47){
  ...
  if(pheno!='HCM'){
    write_xlsx(pleio_pass,str_c('1_MR/1_data/gwas_associations/1_pleio_pruned/',pheno, '_gwas_associations.xlsx'))
  } else{
    write_xlsx(pleio_pass,str_c('1_MR/1_data/gwas_associations/1_pleio_pruned/',pheno, '_to_', pheno2,'_gwas_associations.xlsx'))
  }
...
pwalk(list(instrument_files, phenotypes, redflags),~snp_pleio_filter(..1,..2,'1_MR/3_output/1_MR_QC/pleiotropy_check/',..3))

#Run the exceptions for BMI -> HCM and DBP -> HCM
snp_pleio_filter('1_MR/1_data/gwas_associations/tadros25_hcm_nonMTAG_associations.xlsx','HCM','1_MR/3_output/1_MR_QC/pleiotropy_check/',bmi_exception ,'BMI')
snp_pleio_filter('1_MR/1_data/gwas_associations/tadros25_hcm_nonMTAG_associations.xlsx','HCM','1_MR/3_output/1_MR_QC/pleiotropy_check/',dbp_exception ,'DBP')

#Run for internal HCMR and UKB GWAS summstats
pwalk(list(hcmr_instrument_files, hcmr_phenotypes, hcmr_redflags),~snp_pleio_filter(..1,..2,'1_MR/3_output/1_MR_QC/pleiotropy_check/',..3))
pwalk(list(ukb_instrument_files, ukb_phenotypes, ukb_redflags),~snp_pleio_filter(..1,..2,'1_MR/3_output/1_MR_QC/pleiotropy_check/',..3))

#Run for the HCM subgroups
pwalk(list(subgroup_hcm_instrument_files, subgroup_hcm_phenotypes, subgroup_hcm_redflags),~snp_pleio_filter(..1,..2,'1_MR/3_output/1_MR_QC/pleiotropy_check/',..3))
...
v2g_saver_plotter <- function(v2g,og_tb, pheno, output_path='1_MR/3_output/1_MR_QC/v2g_check/' ){
...
#'   non_hcm_instruments_path = "1_MR/1_data/gwas_associations/1_pleio_pruned/BMI_gwas_associations.xlsx",
...
#'   non_hcm_instruments_path = "1_MR/1_data/gwas_associations/1_pleio_pruned/SBP_gwas_associations.xlsx",
...
create_mr_yaml_config <- function(non_hcm_phenotype,
                                  ...
                                  hcm_instruments_path = '1_MR/1_data/gwas_associations/1_pleio_pruned/HCM_to__gwas_associations.xlsx') {
  ...
  static_output_path_val <- '1_MR/3_output/2_MR/'
  ...
  # Example: '1_MR/1_data/gwas_associations/1_pleio_pruned/HCM_to_BMI_gwas_associations.xlsx'
...
hcmr_yaml_outputpath <- str_c('1_MR/1_data/input_configs/',hcmr_phenotypes, '_and_HCM.yaml')
ukb_yaml_outputpath <- str_c('1_MR/1_data/input_configs/',ukb_phenotypes, '_and_HCM.yaml')
