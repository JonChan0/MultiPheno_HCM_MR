...
snp_pleio_filter <- function(og_path, pheno, output_path,red_flags='',pheno2='', n_cutoff=50){
  ...
  print(str_c('For phenotype ', pheno, ' of ', length(unique(og_snps$rsID)), ' instruments, ', 'total of ',length(output_snps), ' instruments excluded with ',nrow(n_failed_snps), ' instruments excluded due to excessive pleiotropy with >', n_cutoff, ' traits and ', nrow(redflag_snps), ' instruments excluded due to association with traits linked to the oucome e.g. ', str_c(red_flags, collapse='|')))
